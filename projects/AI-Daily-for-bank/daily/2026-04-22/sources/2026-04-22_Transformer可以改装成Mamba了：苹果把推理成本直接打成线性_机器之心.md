...
> 原文链接：https://mp.weixin.qq.com/s/NLTYnIWvhPffpQKIM3qIdw
> 公众号：机器之心
...
，让 Transformer 先变成一个更简单、更接近 Mamba 的版本。再从这个中间版本，转成 Mamba。

用来把 Transformer 里原本

计算很贵的 Softmax Attention

，换成一个

更省算力的 Mamba 模块

。中间加了一个过渡形态：先把 Attention 变成一种

学出来的线性 Attention（Hedgehog）
...
再通过余弦相似度（cosine similarity）蒸馏，让这个新结构在输出上对齐 Transformer。这一步做完，相当于得到一个
...
整套方法的关键不在某一个技巧，而在这条路径本身：先让两种模型在
...
过去几年积累的大量 Transformer 模型，并不需要全部推倒重来，而是有机会通过一套流程，被
