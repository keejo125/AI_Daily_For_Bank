...
为什么会这样？这篇论文给出了两个诊断。
一个是「计算无分化」
。同一组网络反复使用，又缺少明确的轮次信息，很难在不同阶段形成分工，容易沿用相似的计算方式。
另一个是「信息过载」
。同一份hidden state既要保存原始输入，又要装下前几轮结果，还得承担当前计算。长期记忆和临时加工全挤在一起，最后谁也干不好。
...
接着，MeSH给资料柜配了两个管理员。
一个是Write Router
，负责决定这一轮的新结果，应该按什么权重分摊到各个记忆槽。
另一个是Read Router
，负责在下一轮开始前，按不同权重读取各个槽位，重新组合成新的hidden state。
而且，这些管理员各有各的工作方式。
