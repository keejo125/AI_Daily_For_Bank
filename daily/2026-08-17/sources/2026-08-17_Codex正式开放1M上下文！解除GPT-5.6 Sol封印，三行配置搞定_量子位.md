...
digest: |
  OpenAI研究员Tibo披露只需3行配置即可在Codex中解锁GPT-5.6 Sol的1M上下文（约90万token起自动压缩）。官方文档本就支持但默认不开启，因超长上下文会显著推高token消耗且长上下文利用率下降（MRCR v2在512K-1M区间从91.5%降至73.8%）。OpenAI将选择权交还用户，但提醒谨慎使用。
