...
digest: |
  研究团队提出ARA（Agent-Native Research Artifacts）格式替代传统PDF论文，被IEEE Spectrum重点报道。ARA将研究重组为机器可直接操作的知识包，包含科学逻辑层、可执行代码层、探索图层和证据层四个层次，保留论文中因叙事税而被删减的失败路线和实验过程。测试显示使用ARA的AI理解准确率达93.7%（传统材料仅72.4%），复盘失败准确率81.4% vs 15.7%。在15篇论文的复现测试中ARA难度加权成功率64.4% vs 57.4%。论文分析24008次AI Agent运行中发现90.2%的资金成本消耗在失败探索上，而传统论文几乎不保存这些信息，ARA正是为了解决这一痛点。
---
