...
digest: |
  智东西报道，阿里千问大模型团队开放 Qwen3.8-2.4T-A95B 模型权重，这是千问首次将 Max 级别旗舰模型对外开放权重。该模型总参数 2.4 万亿、每 Token 激活 950 亿，原生支持 262144 Token 上下文（可扩展至 101 万），采用 Qwen3.5 架构，在编程、办公、科研、长周期智能体任务上提升明显，默认思考模式运行。Unsloth AI 以动态 1-bit 分层量化将其 4.9TB 体积压缩至 397GB（缩减 91%），设备内存+显存 410GB 以上即可本地运行。基准上，Qwen3.8-Max 在 PaperBench 达 93.0 分（高于 GPT-5.6 Sol、Fable 5、Claude Opus 4.8），OSWorld-Verified 86.1 分居首，但 TerminalBench 2.1、SWE-bench Pro 等仍落后；其重点是可脱离人工、自主完成跨天数周的长周期任务。
