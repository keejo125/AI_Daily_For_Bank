...
digest: |
  Meta AI Research推出Muse Glimmer——300亿参数开放权重模型，采用Apache 2.0许可。该模型专为始终在线的本地工作流设计，使开发者无需云API即可在消费级GPU与工作机上运行自主智能体、复杂工具调用与本地编码，支持视觉与工具调用。
---

# 本地运行、支持视觉与工具调用：Meta 开源智能体模型

> 原文链接：https://www.infoq.cn/article/aGfkSN1YlmLrUQMPea9L
> 来源：InfoQ
...
Logit 蒸馏（预训练）：该模型使用相匹配的预训练数据集组合，从 Muse Spark 迁移基础推理能力。中期训练：使用包含复杂推理轨迹、交错文本与图像数据以及多步工具调用轨迹的长上下文序列扩大训练规模。后训练对齐：结合监督微调（SFT）、同策略蒸馏和强化学习（RL），优化代码生成、工具使用和结构化规划等多个领域的性能。
...
原文链接：https://www.infoq.com/news/2026/08/meta-muse-glimmer/"