...
digest: |
  腾讯 8 月 28 日发布并开源新一代大模型混元 Hy4 preview：总参数 770B（激活 49B），上下文从 256K 扩展至 1M，已接入 WorkBuddy、CodeBuddy、元宝与 ima。极客公园实测三项长程任务：用 3 分 27 秒完成 6 笔费用材料交叉核验（核准 4294 元、核减 230 元）；32 分钟用原生 Canvas+TypeScript 做出可运行小游戏《深海进化论》并自跑 29 项测试；1 小时 39 分开发 Three.js 3D 竞速游戏《午夜港口》，首版白屏后修复打包交付。腾讯称其首次参与自身训练方法与算子优化，推理吞吐较基线提升 31.8%。
---
...
这种「发现问题&mdash;运行实验&mdash;根据结果继续修改」的能力，是否也能出现在普通用户的任务里？
...
这个 Case 里，模型做到了「需求拆解&mdash;代码生成&mdash;运行测试&mdash;发现问题&mdash;修复问题&mdash;再次验证」串成一条完整链路。放在「原生 Canvas、零第三方游戏引擎、32 分钟完成」的前提下，《深海进化论》能开始、能玩、能失败、能重开，也留下了一套能继续修改和验证的工程文件。
...
三项测试呈现出了一条相似的轨迹：Hy4 preview 并非不会犯错&mdash;&mdash;费用审核选错过制度版本，3D 游戏第一次交付甚至无法打开。但与只会生成答案的模型相比，它更重要的变化是能够在错误出现后继续工作：寻找关联材料、运行测试、定位问题，并在已有结果上迭代，而不是重新开始。
