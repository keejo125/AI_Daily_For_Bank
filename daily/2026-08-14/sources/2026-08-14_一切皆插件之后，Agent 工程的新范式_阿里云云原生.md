...
digest: |
  阿里云云原生解析 DeepSeek Harness 提出的“一切皆插件”Agent 工程新范式：模型、工具、会话、记忆、UI、Agent 主循环等全部做成可插拔组件，以“日志为唯一真相源”实现每次运行可回放、分叉、审计，降低 Agent 体验与成本优化复杂度。对比 LangChain 等“向上封装”框架在模型黑盒外叠加框架黑盒、定位困难；Pi Agent 走“最小核心+按需扩展”；DeepSeek Harness 插件化更彻底，分 Bundle→Profile→Patch→Overlay 四层叠加成插件树。该透明架构使效果调优（换循环/上下文注入）与成本调优（看清每次 Token 来源）可操作，但目前仍为开发者预览阶段。
---
