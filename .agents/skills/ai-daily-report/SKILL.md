...
**技术 vs 非技术内容判定**（关键！）：
- ✅ **技术内容**：模型发布、算法研究、工程实践、开发工具、框架更新、安全研究
  - 归入国际/国内（根据主体归属）或同业（金融机构应用）
- ❌ **非技术内容**：商业纠纷、法律诉讼、融资并购、财报数据、人事变动、活动宣传、综合资讯汇总
  - 一律归入"其他"
  
**典型案例分析**：
- "曝苹果OpenAI关系破裂，可能上法庭" → 法律纠纷，非技术 → **其他**
- "腾讯研究院AI速递 20260515" → 综合资讯汇总，多主题混合 → **其他**
- "最强黑客大模型，不再是Mythos" → 微软MDASH系统技术创新 → **国际**
- "越懂你，越危险？MemPrivacy揭示AI记忆下一站" → MemTensor(上海AI实验室)+荣耀技术研究 → **国内**
...
7. **source_file 模糊匹配**：`generate_html.py` 使用 `normalize_title_for_match()` 函数统一引号格式（U+201C/U+201D → U+0022），然后进行三级匹配策略：
   - 精确匹配：完整的 title + source
   - 次优匹配：title 前20个字符 + source
   - 兜底：只有一个文件时直接返回
   - ⚠️ **注意**：如果 classification.json 中的 source_file 为空，程序会自动尝试查找匹配文件
8. **多来源整合规范**：
   - 同一事件的多篇报道需要整合展示
...
15. **⭐ 文件路径与引号兼容性**
    - classification.json 中的 title 应使用**英文直引号**（U+0022 `"`）
    - 实际文件名可能包含**中文弯引号**（U+201C `"`, U+201D `"`）或**不换行空格**（U+00A0）
    - `generate_html.py` 的 `normalize_title_for_match()` 函数会自动处理：
      - 引号标准化：U+201C/U+201D → U+0022，U+2018/U+2019 → U+0027
      - 空格标准化：U+00A0 → U+0020
    - 如果 source_file 字段有特殊字符导致 JSON 转义困难，可设置为空字符串 `""`，程序会自动查找匹配文件
    - ⚠️ **注意**：合并文章的 source 通常设置为"多源综合"，可能导致文件匹配失败，这是正常的
